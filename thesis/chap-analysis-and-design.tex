...
on a fixed threshold at rung~0 and an archive-relative threshold at
rung~1. The promotion mechanism of this scheduler is based on three
...
ASHA reuses Hyperband's parameters: \texttt{--hyperband\_eta},
\texttt{--hyperband\_budgets}, \texttt{--hyperband\_initial\_children}.
It extends Hyperband by removing the per-rung synchronisation barrier.

\paragraph{Genetic Algorithm (GA).}
The Genetic Algorithm (GA) scheduler is a blind-evolution control: it
preserves DGM's three-stage Baseline evaluation pipeline but strips
error-log context from child generation. It is used to test whether
...
\texttt{--ga\_tournament\_k} controls the tournament size at parent
selection (activated by setting
...
\begin{itemize}
  ...
  \item \textbf{Initial archive}: bootstrapped with
    \texttt{test\_swebench.py --benchmark swe\_verified\_mini
    --full\_mini} into \texttt{initial\_swe\_verified\_mini/}.
  ...
  \item \textbf{Children per generation}: \texttt{2}
    (\texttt{--selfimprove\_size 2}) for Baseline and GA; Hyperband and
    ASHA spawn \texttt{--hyperband\_initial\_children 15} instead.
  \item \textbf{Per-generation task budget cap}: \texttt{100}
    (\texttt{--generation\_task\_budget\_total 100}).
  \item \textbf{Archive update policy}: \texttt{keep\_all}
    (\texttt{--update\_archive keep\_all}).
...
A sample run command for Hyperband:
...
The other three schedulers use identical flags with
\texttt{--scheduler} swapped. Baseline drops the three
\texttt{--hyperband\_*} flags; GA drops them and adds
...
\texttt{dgm\_metadata.jsonl}). Primary metrics for the research
question are marked \emph{primary}; the remainder are
\emph{diagnostic}.

\paragraph{Per-child metrics.}
\begin{itemize}
  \item \texttt{accuracy\_score} (\emph{primary}). Defined as
    \texttt{total\_resolved\_instances / total\_submitted\_instances}
    (a rate in $[0, 1]$). Empty patches are tracked separately in
    \texttt{total\_emptypatch\_ids} and excluded from the denominator;
    see Section~\ref{sec:threats} for the bias this introduces under
    minimax-m2.5. Direct measure of agent effectiveness on the tasks it
    attempted.
  \item \texttt{is\_compiled} (\emph{diagnostic}). A boolean flag
    (\texttt{is\_compiled\_self\_improve} in
    \texttt{utils/evo\_utils.py}) that is \texttt{True} when the child
    (a) produced at least one non-empty patch, i.e.\
    \verb|len(total_resolved_ids) + len(total_unresolved_ids) > 0|, and
    (b) has \texttt{compile\_failure\_reason == None}. Tracks wasted
    budget on children whose patch never compiled or was empty.
  ...
  \item \texttt{token\_usage\_total} (\emph{primary, cost component}).
    Sum of input + output tokens consumed by the child across all LLM
    calls (mutation generation, evaluation iterations). Read directly
    from \verb|metadata['token_usage_total']|. Multiplied by the
...
\begin{itemize}
  \item \texttt{best\_child\_score} (\emph{primary outcome}).
    $\max(\texttt{accuracy\_score})$ over children whose
    \texttt{evaluated\_task\_count} $\geq$ \texttt{full\_budget\_size}.
    Single-number summary of the best agent produced in the
    generation. Tracks DGM's progress over generations.
...
without any significance tests or confidence intervals. Within each
run, all the metadata is loaded and the primary metrics are plotted as
a function of generation: \texttt{best\_child\_score},
\texttt{cost\_per\_gen}, and
\texttt{evaluation\_budget\_tasks\_consumed}. Diagnostic metrics
(\texttt{generation\_wall\_clock\_seconds},
\texttt{children\_compiled\_count}, \texttt{archive\_size},
\texttt{promoted\_from\_rung} distribution) are plotted alongside for
context. The four schedulers are then compared on the same axes to
...
Chapter~\ref{chap:results}, Section~[N.1]. The best-of-run score is
...
trajectories appear in Section~[N.2]. The cost-vs-score comparison,
which is the central result for the research question, appears in
Section~[N.3].
...
\begin{itemize}
  ...
  \item \texttt{accuracy\_score} denominator excludes empty patches;
    minimax-m2.5 can game the metric by emitting empty patches on tasks
    it cannot solve.
