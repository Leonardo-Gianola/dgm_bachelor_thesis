...
\begin{itemize}
  \item \textbf{\texttt{DGM\_outer.py}}: changed the default benchmark to
    ...
    default). The last changes were general bug fixes.
  \item \textbf{\texttt{self\_improve\_step.py}}: four big changes. Added
    ...
    \texttt{utils/docker\_utils.py}, see Section~\ref{sec:guards}.) Last,
    fixed bugs introduced by these changes.
...
The rung structure is: \textbf{Rung 0 (small)} evaluates all children,
promoting those that resolve at least 40\% of small-stage tasks;
\textbf{Rung 1 (medium)} takes the Rung-0 survivors and promotes those
whose score is greater than or equal to the full-evaluation threshold;
\textbf{Rung 2 (full)} takes the remaining children, evaluates them on the
full suite, and keeps all of them. Every stage promotion has two steps:
(a) a quality gate filtered by the threshold (Rungs 0 and 1), then (b) a
...
3--10, not tasks 1--10. This is a slight deviation from textbook
Hyperband.
...
class GAScheduler(BaselineScheduler):
    ...
    # run_generation / rung logic: inherited unchanged from BaselineScheduler
\end{lstlisting}
...
does not exits, or any required flag is absent, the child is marked
