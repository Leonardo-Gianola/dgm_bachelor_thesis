...
\begin{figure}[tb]
  ...
  % spine flow (arrows drawn full; labels offset so they never clip the heads)
  \draw[flow] (orch) -- (gen);
  \draw[flow] (gen)  -- (eval);
  \draw[flow] (eval) -- (store);
  \node[lbl, anchor=west] at ($(orch.south)!0.5!(gen.north)+(2.5mm,0)$)   {spawn child};
  \node[lbl, anchor=west] at ($(gen.south)!0.5!(eval.north)+(2.5mm,0)$)   {code patch};
  \node[lbl, anchor=west] at ($(eval.south)!0.5!(store.north)+(2.5mm,0)$) {resolved / unresolved};

  % calls to external services (arrows full; labels above, clear of heads)
  \draw[flowd] (gen.east)  -- (or.west);
  \draw[flowd] (eval.east) -- (swe.west);
  \node[lbl, anchor=south] at ($(gen.east)!0.45!(or.west)+(0,0.6mm)$)   {LLM call};
  \node[lbl, anchor=south] at ($(eval.east)!0.45!(swe.west)+(0,0.6mm)$) {run tasks};
